...
for w in w_list:
    ...
    for st_cond in sstp_cond:
        ...
        plt.grid()
        plt.plot(fnc.variables["RH"][:]*100 , z, label=st_cond)
        plt.legend()
        ...
        ax.set_ylabel('Z [m]]')
        plt.grid()
        plt.plot(fnc.variables["cloud_m0"][:] , z, label=st_cond)
        plt.legend()
        plt.title("0 moment for updraft velocity="+str(w)+"m/s")
        if not os.path.exists(output_folder):
            subprocess.call(["mkdir", output_folder])
    plt.savefig(os.path.join(output_folder, "plot_profiles_onesim" + str(st_cond)+"updraft_velocity"+ str(w)+".svg"))
