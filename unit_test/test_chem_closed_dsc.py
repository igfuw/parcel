...
def test_chem_closed():
    ...
    parcel(dt = dt, z_max = z_max, outfreq = outfreq,\
           ...
           out_bin = ["chem:0/1/1/lin/wet/O3_a,H2O2_a,SO2_a,HSO3_a,SO3_a"],)
    ...
    #final gas phase = current gas phase + mass dissolved into droplets
    end = f.variables["SO2_g"][-1] + \
          aq_to_g(f.variables["SO2_a"][-1],  f.variables["rhod"][-1], f.variables["T"][-1], cm.M_SO2, f.variables["p"][-1]) + \
          aq_to_g(np.squeeze(f.variables["chem_HSO3_a"][-1]), f.variables["rhod"][-1], f.variables["T"][-1], cm.M_HSO3, f.variables["p"][-1]) + \
          aq_to_g(np.squeeze(f.variables["chem_SO3_a"][-1]),  f.variables["rhod"][-1], f.variables["T"][-1], cm.M_SO3, f.variables["p"][-1])
