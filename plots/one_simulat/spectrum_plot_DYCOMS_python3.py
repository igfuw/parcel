...
def plot_spectrum(data, data2,  outfolder):
    ...
    for t in range(data.variables['t'].shape[0]):
        ...
        fig.savefig(os.path.join(my_path, 'plot_spec_' + str("%03d" % t) + '_DYCOMS.png'))
        plt.clf()
        plt.cla()
        plt.close()
...
#Stratocumulus - DYCOMS
    ...
    # out_bin = ast.literal_eval(getattr(data, "out_bin")) NIE działa
...
#    rd = data.variables["wradii_r_wet"][:] # left bin edges
    ...
    np.savetxt('Dycoms_for _HP.csv', [p for p in zip(rd, data.variables['wradii_m0'][0,:])], delimiter=',', fmt='%s')
    ...
    plt.legend()
    plt.grid(True,  linestyle='-.')
    fig1.savefig(os.path.join(my_path, 'init_spectrum_DYCOMS.png'))
    plt.clf()
    plt.cla()
    plt.close()
...
def main():
    ...
    print(r_init)
    outfile = "test_spectrum_DYCOMS.nc"
    outfile2 = "test_spectrum2_DYCOMS.nc"
    ...
    # plotting
#    plot_spectrum(data, data2, outfolder="/home/piotr/Piotr/IGF/parcel3/parcel/plots/wyniki_python3/DYCOMS")
    # plotting
    plot_init_spectrum(data, outfolder="/home/piotr/Piotr/IGF/parcel3/parcel/plots/wyniki_python3/DYCOMS")
