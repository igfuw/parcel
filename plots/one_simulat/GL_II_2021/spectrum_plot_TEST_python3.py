...
def plot_spectrum(data, data2,  outfolder):
    ...
    for t in range(data.variables['t'].shape[0]):
        ...
        fig.savefig(os.path.join(my_path, 'plot_spec_' + str("%03d" % t) + '_TEST.png'))
        plt.clf()
        plt.cla()
        plt.close()
...
#    rd = data.variables["wradii_r_wet"][:] # left bin edges
    ...
    plt.legend()
    plt.grid(True,  linestyle='-.')
    fig1.savefig(os.path.join(my_path, 'init_spectrum_TEST.png'))
    plt.clf()
    plt.cla()
    plt.close()
...
#    gstdev2  = 1.25
    # run parcel run!
    parcel(dt = 1, T_0 = T_init, p_0 = p_init, RH_0 = .95, sstp_cond =1, z_max = H, w=W,  sd_conc = SD, outfreq = 1, aerosol = '{"ammonium_sulfate": {"kappa": 0.61, "mean_r": [ 0.24e-6], "gstdev": [1.58], "n_tot": [90e6]}}', outfile = outfile, out_bin = out_bin)
...
#    # doing plotting
    plot_init_spectrum(data, outfolder="/home/piotr/Piotr/IGF/parcel3/parcel/plots/wyniki_python3/GL_II_2021/TEST")
