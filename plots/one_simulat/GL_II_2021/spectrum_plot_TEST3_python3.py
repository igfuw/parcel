...
def plot_spectrum(data, data2,  outfolder):
    ...
    for t in range(data.variables['t'].shape[0]):
        ...
        plt.step(rd, nd,  label="dry radius")
        ...
        fig.savefig(os.path.join(my_path, 'plot_spec_' + str("%03d" % t) + '_TEST3.png'))
        plt.clf()
        plt.cla()
        plt.close()
...
#    rd = data.variables["wradii_r_wet"][:] # left bin edges
    ...
    np.savetxt('TEST3_for _HP.csv', [p for p in zip(rd, data.variables['wradii_m0'][0,:])], delimiter=',', fmt='%s')
    ...
    plt.legend()
    plt.grid(True,  linestyle='-.')
    fig1.savefig(os.path.join(my_path, 'init_spectrum_TEST3.png'))
    plt.clf()
    plt.cla()
    plt.close()
...
def main():
    ...
    outfile = "test_spectrum_TEST3.nc"
    outfile2 = "test_spectrum2_TEST3.nc"
...
#    gstdev2  = 3.25
    # run parcel run!
    parcel(dt = 1, T_0 = T_init, p_0 = p_init, RH_0 = .98, sstp_cond =1, z_max = H, w=W,  sd_conc = SD, outfreq = 1, aerosol = '{"ammonium_sulfate": {"kappa": 0.61, "mean_r": [0.01e-6, 0.44e-6], "gstdev": [1.58, 2.25], "n_tot": [90e6, 15e6]}}', outfile = outfile, out_bin = out_bin)
    parcel(dt = 1, T_0 = T_init, p_0 = p_init, RH_0 = .98, sstp_cond =10, z_max = H, w=W,  sd_conc = SD, outfreq = 1, aerosol = '{"ammonium_sulfate": {"kappa": 0.61, "mean_r": [0.01e-6, 0.44e-6], "gstdev": [1.58, 2.25], "n_tot": [90e6, 15e6]}}', outfile = outfile2, out_bin = out_bin)

    data = netcdf.netcdf_file(outfile, "r")
    data2 = netcdf.netcdf_file(outfile2, "r")
    # plotting
    plot_spectrum(data, data2, outfolder="/home/piotr/Piotr/IGF/parcel3/parcel/plots/wyniki_python3/GL_II_2021/TEST3")
#    # doing plotting
    # plot_init_spectrum(data, outfolder="/home/piotr/Piotr/IGF/parcel3/parcel/plots/wyniki_python3/GL_II_2021/TEST3")
